...
const CurriculumItem = ({ item, isHighlighted }) => (
  <div className={clsx(styles.curriculumItem, { [styles.highlighted]: isHighlighted })}>
...
const CourseDetails = ({ course, expanded }) => (
  <div className={clsx(styles.courseDetailsWrapper, { [styles.expanded]: expanded })}>
    <h3 className={styles.courseDetailsTitle}>Curriculum</h3>
    <div className={styles.courseCurriculum}>
      {course.curriculum.map((item, index) => (
        <CurriculumItem key={index} item={item} isHighlighted={index % 2 === 0} />
      ))}
    </div>
  </div>
);
function Details() {
  ...
  return (
    ...
    <div className={styles.detailsContainer}> 
        ...
        <div className={clsx(styles.courseHeader, styles.someOtherCondition && styles.additionalClass)}>
    ...
    <div className={styles.courseImages}>
      {course.images.map((image, index) => (
        <div key={index} className={styles.courseImageWrapper}>
          <img src={image.src} alt={image.alt} className={styles.courseImage} />
        </div>
      ))}
    </div>
